...
```{r}
load(file = '../data/preprocessed_data/all_bounding_boxes_three_kids_preprocessed.RData')
```


### Randomly sample some subset of the data
```{r}
num_to_sample = .1*length(bb_all_three$frame)
bb_all_three_sample <- bb_all_three %>%
  sample_frac(.1)
```

```{r}
save(file = '../data/preprocessed_data/all_bounding_boxes_three_kids_preprocessed_small_sample.RData', bb_all_three_sample)
```

```{r}

bb_all <- bb_all_three_sample
```

```{R}
ggplot(bb_all_three_sample) +
...
```{r}
hands_by_size_by_frame <- hands %>%
  group_by(vid_name, frame, age_days, child_id) %>%
  summarize(num_hands = n(), avg_area = mean(height*width), avg_x = mean(center_x), avg_y = mean(center_y))
