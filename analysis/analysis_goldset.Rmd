...
# load('../data/preprocessed_data/openpose_detections_filtered_2020-01-29.RData')
# vid_info <- d_all %>%
  # select(vid_name, frame, age_days, child_id)
# save(vid_info, file = '../data/preprocessed_data/vid_info.RData')
load('../data/preprocessed_data/vid_info.RData')


# load('../data/raw_data/vid_info.RData')
...
```{r}
bounding_boxes = "../data/raw_data/gold_sample_bounding_boxes.csv" 
...
dir.create(paste0('bbcheck_height/'))
# unique_images <- bb_check$full_image_path[300:350]

bb_check_subset <- bb_check %>%
  filter(height>.8) %>%
  filter(label=='hand')

unique_images = unique(bb_check_subset$full_image_path)
for (image in unique_images){
  ...
  rect(a$x_pos, a$y_pos, a$x_pos+a$width_x, a$y_pos - a$height)
  # text(a$x_pos*680, a$y_pos*480, a$top)
  # text(a$x_pos*680, a$y_pos*480, a$y_pos)
  annotated_file = paste0('bbcheck_height/',unique(a$vid_name_short), unique(a$frame_ind), 'OP-big-hands.jpg')
...
# ggplot(bb_hand_only, aes(x=left, y=top)) +
#   geom_point() + 
