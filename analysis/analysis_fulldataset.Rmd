...
### Now do the same for bounding box data
```{r}
...
levels(bb_all$vid_name) <- all_video_names$vid_name

bb_all <- bb_all %>%
  filter(!vid_name %in% flipped_videos$vid_name) 
  left_join(vid_info, by='vid_name') 
  # 
...
bb_all = bb_all %>%
  ...
  mutate(age_day_bin = cut(age_days, bins, labels=round(x/30,1))) %>%
  mutate(age_day_bin = as.numeric(as.character(age_day_bin)))

detections_cropped <- bb_all %>%
  filter(height <.6) %>%  ## cut off bottom part of frame
  filter(mean_conf > 0) %>% ## look at detections
  group_by(vid_name, frame, age_days, child_id) %>%
  summarize(hand_detected = (length(label=="hand")>0), face_detected = (length(label=="face")>0)) %>%
  distinct(vid_name, frame, age_days, child_id, face_detected, hand_detected)
```

### Now plot faces-hands for the cropped analysis
```{r}
summary_by_age_cropped <- detections_cropped %>%
  group_by(age_day_bin, child_id) %>%
  summarize(num_detect = length(face_detected), prop_faces = sum(face_detected) / num_detect,  prop_hands = sum(hand_detected) / num_detect) 

ggplot(summary_by_age_cropped, aes(x=age_day_bin, y=prop_faces - prop_hands, size=num_detect, col=child_id)) +
  geom_point(alpha=.8) +
  geom_smooth(span=10) + 
  theme_few(base_size=18) +
