...
#TODO: return the paths to the batches in order as a list
def save_sample_imgs(sample_df):
    ...
    for df_num, s in enumerate(range(0, len(sample_df), BATCH_SIZE)):
        ...
        imgs = cut.apply(_get_img, axis=1).values
        print(f'Saving imgs #{df_num}')
        np.save(os.path.join(OUTPUT, 'sample_imgs_{df_num}.npy'), imgs)
...
#NOTE: This script does not currently work on Sherlock for me, as the OpenCV version cannot do 
# imshow(). To use this on Sherlock in a notebook, modify to use plt.imshow() instead (this will be 
