...
Overall, these results largely validate prior cross-sectional findings within more naturalistic learning contexts and individual children. These results also highlight some of the challenges of automated coding of infant egocentric headcam data, raising key areas for future computational and empirical work. At the same time, these findings highlight the promise of using automated annotations to analyze large, naturalistic video datasets, advancing the overarching goal of building generalizable conclusions about the statistics of the social information in the infant view across a wide variety of populations. 
...
We next examined the degree to which including detections that the model was more or less confident in changed overall model performance for face/hand detection. When we restricted detections to high-confidence face/hand detections (>.5 confidence, default threshold for visualization in OpenPose) was not beneficial -- improving precision but dramatically impairing recall and thus overall performance: the F-score for high-confidence face detections was `r round(face_performance_hc[3],2)`, with a precision of `r round(face_performance_hc[1],2)` and recall of `r round(face_performance_hc[2],2)`; for high-confidence hand detections, the F-score was `r round(hand_performance_hc[3],2)`, with a precision of `r round(hand_performance_hc[1],2)` and recall of `r round(hand_performance_hc[2],2)`. This is consistent with findings from Long et al., 2021, where restricting analyses to high-confidence detections also impaired detector performance. Further analyses sweeping across the entire range of possible confidence detection thresholds revealed that including all detections was the most beneficial for optimizing the rate of true positives (see Appendix Figure A4 for full ROC curves for both face and hand detections).

### Accuracy across child, location, and age
Finally, we examined whether model accuracy for hands and faces varied across age or between child and the location in which the videos were filmed (e.g., kitchen vs. living room). Some of the videos at taken at older ages for child A did seem to show lower hand detection accuracy, perhaps due the presence of child's hands; however, there were also fewer videos at these time points (see plots for age/child in Appendix, Figure A5). We did not find any discernible trends in face/hand detection accuracy across different filming locations (see Figure A6). However, we did still see considerable variation in model accuracy across different videos, suggesting that particular recording activities, lighting conditions, or situations could cause conditions that would impair detector performance. For example, challenging lighting conditions in a poorly-lit room or very bright outdoor day could lead to overall lower detection accuracy, as could activities where caregivers are moving rapidly, leading to more blurred views of faces that can be difficult to detect. 
...
first_panel <- ggplot(all_detections, 
  ...
  # scale_color_manual(values = c("#1266a2","#1b91e6", "#CB4B16","#eb7443"), name = "") +
...
second_panel <- ggplot(centered, 
  ...
  facet_grid(cols = vars(child_name)) + 
  theme_few(base_size=10) +
  # scale_color_viridis(option = "turbo", discrete=TRUE, begin=0.05, end=.85, name="") +
  # scale_color_manual(values = c("#92c5de","#ef8a62"), name = "") +
      scale_color_manual(values = c("#1b91e6","#eb7443"), name = "") +
  theme(legend.text=element_text(size=10)) +
  guides(size = "none") +
  theme(legend.position="none", aspect.ratio=.75)  
  # ggtitle('B.')
...
third_panel <- ggplot(detailed, 
  ...
  facet_grid(cols = vars(child_name)) + 
  theme_few(base_size=10) +
  # scale_color_viridis(option = "viridis", discrete=TRUE, begin=0.3, end=.8, name="") + 
  # scale_color_manual(values = c("#751098","#142c83"), name = "") +
  scale_color_manual(values = c("#a21266","#142c83"), name = "") +
  theme(legend.text=element_text(size=10)) +
  guides(size = "none") +
  theme(legend.position="none", aspect.ratio=.75) 
  # ggtitle('C.')
...
cowplot::plot_grid(first_panel, second_panel, third_panel, labels='auto', nrow=3, label_fontface = "bold", align = "hv", axis = "b")
ggsave(filename='figs_publication_final/figure2_updated.pdf', width = 7.2, height=8, dpi=300)
```
...
ggplot(face_hand_contingency %>% filter(num_frames_total>20000), 
       aes(x=age_day_bin, y=prop, 
           col=child_id)) +
  ...
  scale_color_viridis(option='viridis', discrete=TRUE, begin=.2, end=.8) +
  guides(size = "none") +
  theme(legend.text=element_text(size=6), legend.position = "none")

ggsave('figs_publication_final/figure3_face_hand_contingency.pdf', width=3.5, height=2.625, units='in')
```
...
ggplot(all_to_plot, aes(x = Location, y = mean, col=region)) + 
  geom_point(aes(x=Location, y=prop, size=log10(num_detect)), alpha=.05, position = position_dodge(width=.6)) +
  geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper, linetype=region),position = position_dodge(width=.8), size=.3) +
  coord_flip() + 
  ...
  # scale_color_manual(values = c("#1a2591", "#7fb8f1", "#b9221f","#ea5e5c"), name = "") +
  scale_color_manual(values = c("#1266a2","#1b91e6", "#CB4B16","#eb7443"), name = "") +
  theme(legend.text=element_text(size=8)) +
  guides(size = "none") 
  
ggsave('figs_publication_final/figure4_dets_by_location.pdf', width=7.2, height=5, units='in')
```
...
ggsave('figs_publication_final/figure5_size_by_age.pdf', width=3.5, height=2.625, units='in')
```

```{r}
body_size_summary = summary(lmer(data = all_vid_bbs %>% filter(label=='pose'), log(avg_area) ~ scale(age_days) + scale(num_detect) + (1|child_id)))

faces_size_summary = summary(lmer(data = all_vid_bbs %>% filter(label=='face'), log(avg_area) ~ scale(age_days) + scale(num_detect) + (1|child_id)))

hands_size_summary = summary(lmer(data = all_vid_bbs %>% filter(label=='hand'), log(avg_area) ~ scale(age_days) + scale(num_detect) + (1|child_id)))
...
```{r}
eye_dist_summary = summary(lmer(data = all_vid_data, avg_eye_distance ~ scale(age_days) + scale(num_frames) + (1|child_id)))
```
...
```{r}
x_vs_y_summary = summary(lmer(data=CV_faces, CV ~ scale(age_day_bin) + position + scale(num_detect) +  (1|child_id)))
...
cowplot::plot_grid(eye_dist_plot, CV_faces_plot, labels='AUTO', rel_widths=c(1,2))
ggsave('figs_publication_final/figure6_detailed_changes.pdf',units='in',width=7, height=3)
```
...
Supporting this idea, we found that the averages sizes of the people, faces, and hands in the infant view became smaller over development (Figure \ref{fig:avgSizes}). To assess these trends, we fit three linear mixed-effect models, asking whether the average sizes of people, faces, and hands declined with children’s age, and found significant effects in each case (People, B = `r round(body_size_summary$coefficients,4)[2,1]`,
SE = `r round(body_size_summary$coefficients,4)[2,2]`, df = `r round(body_size_summary$coefficients,4)[2,3]`, t = `r round(body_size_summary$coefficients,4)[2,4]`, 
p = `r round(body_size_summary$coefficients,4)[2,5]`;Faces, B = `r round(faces_size_summary$coefficients,3)[2,1]`,
SE = `r round(faces_size_summary$coefficients,4)[2,2]`, df = `r round(faces_size_summary$coefficients,4)[2,3]`, t = `r round(faces_size_summary$coefficients,4)[2,4]`,
p = `r round(faces_size_summary$coefficients,4)[2,5]`); Hands, B = `r round(hands_size_summary$coefficients,4)[2,1]`,
SE = `r round(hands_size_summary$coefficients,4)[2,2]`, df = `r round(hands_size_summary$coefficients,4)[2,3]`, t =`r round(hands_size_summary$coefficients,4)[2,4]`, p = `r round(hands_size_summary$coefficients,4)[2,5]`).
...
There was a trend from larger, closer faces (with a larger interpupillary distance) to smaller faces that were farther away (with a smaller interpupillary distance);  this finding was confirmed in a linear-mixed effect model (B = `r round(eye_dist_summary$coefficients,4)[2,1]`, SE = `r round(eye_dist_summary$coefficients,4)[2,2]`, df = `r round(eye_dist_summary$coefficients,4)[2,3]`, t = `r round(eye_dist_summary$coefficients,4)[2,4]`, P = `r round(eye_dist_summary$coefficients,4)[2,5]`).
...
Faces tended to be more variable in the vertical than their horizontal position (see Figure \ref{fig:faceDetailed}B, Main effect of position type on variability, B = `r round(x_vs_y_summary$coefficients,4)[3,1]`, SE = `r round(x_vs_y_summary$coefficients,4)[3,2]`, df = `r round(x_vs_y_summary$coefficients,4)[3,3]`, t = `r round(x_vs_y_summary$coefficients,4)[3,4]`, P = `r round(x_vs_y_summary$coefficients,4)[3,5]`).
We also found that as children got older, they tended to see faces that varied more in their horizontal -- but not their vertical position -- suggesting that older children might be more likely to see more smaller faces in their periphery (see Figure \ref{fig:faceDetailed}B), (Main effect of age of horizontal position, B = `r round(x_pos_summary$coefficients,4)[2,1]`, SE = `r round(x_pos_summary$coefficients,4)[2,2]`, df = `r round(x_pos_summary$coefficients,4)[2,3]`, t = `r round(x_pos_summary$coefficients,4)[2,4]`, P = `r round(x_pos_summary$coefficients,4)[2,5]`; No main effect of age on vertical position, B = `r round(y_pos_summary$coefficients,4)[2,1]`, SE = `r round(y_pos_summary$coefficients,4)[2,2]`, df = `r round(y_pos_summary$coefficients,4)[2,3]`, t = `r round(y_pos_summary$coefficients,4)[2,4]`, P = `r round(y_pos_summary$coefficients,4)[2,5]`).
...
An analysis based on the location the videos were filmed in further highlights the variability of the social information in view during different activities, showing differences across locations (see Figure 4 and Appendix, Figure A8) and between individual children. 
