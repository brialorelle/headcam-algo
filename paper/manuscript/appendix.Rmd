---
title: "Appendix to ``A longitudinal analysis of the social information in infants' naturalistic
...
gold_sample_no_child_hands <- gold_sample %>%
  ...
  tidyr::replace_na(list(child_hand_seg = FALSE)) %>% # replace NAs with false (frames not in annotations (NAs) did not have hands) 
...
## A1 OpenPose Annotation Examples
```{r include=FALSE}
...
## A2 Density of child vs. adults hands in the visual field
```{r} 
...
ggsave('figs_publication_final/figurea2_handdensity.pdf', width = 7.2, height = 3)
```



## A3 Location of OpenPose False Positives


```{r}
load(here::here('data/preprocessed_data_2022/gold_sample_all_indiivdual_dets.RData'))

ddd <- gold_sample_all_dets %>%  # has bounding box info
  left_join(gold_sample %>% select(frame, vid_name, face_eval_ketan, hand_eval_ketan)) %>%
    mutate(x=left+0.5*width, y=top+0.5*height)  # gold_sample has manual annotations merged

fp_faces <-  ddd %>% 
  filter(face_eval_ketan=="F", label=="face") 

fp_hands <-  ddd %>% 
  filter(hand_eval_ketan=="FP", label=="hand") 
```


Below we visualize the locations of the `r length(fp_faces)` faces in `r length(unique(fp_faces$frame))` frames that OpenPose detected in the gold set frames that were not actually there, as well as the `r length(fp_hands)`  OpenPose false positive hands (in `r length(unique(fp_hands$frame))` frames) from the gold set.
...
```{r density-op-fps, fig.width=6.5, fig.height=3.2, fig.cap="Density plot of the locations of false positives for OpenPose detections for faces (left) and hands (right) in the gold set."}
...
p1 <- fp_faces %>%
  ...
  xlab("Center of OpenPose Detection") + 
  ylab("Center of OpenPose Detection") + 
  ggtitle("Face False Positives") + theme(legend.position = "none") +
  scale_y_reverse() + xlim(-.05,1.05) + ylim(1.05,-.05)
...
ggsave('figs_publication_final/figurea3_op_falsepositives.pdf', width = 7.2, height = 3.4)

```


## A4 Detection threshold estimations
...
roc_data <- gold_sample %>%
  ...
  summarize(tpr_faces = return_tpr(face_eval_ketan), fpr_faces = return_fpr(face_eval_ketan), tpr_hands = return_tpr(hand_eval_ketan), fpr_hands = return_fpr(hand_eval_ketan), f_faces = return_f_score(face_eval_ketan), f_hands = return_f_score(hand_eval_ketan) )
...
```{r roc_curves, fig.width=6.5, fig.height=3.2, fig.env="figure", fig.cap="ROC curves relating the rate of true vs. false positives for both face and hand detections at each level of detector confidence thresholds (lighter values = stricter cutoffs)."}
ggpubr::ggarrange(face_roc, hand_roc, nrow=1)
ggsave('figs_publication_final/figurea4_rocs.pdf', width = 7.2, height = 3)
```

```{r}

f_faces = ggplot(data=roc_data, aes(x=detection_threshold, y=f_faces, color=detection_threshold)) +
  geom_point() +
  theme_few(base_size=10) +
  geom_line() +
  ylim(0,1) +
...
f_hands = ggplot(data=roc_data, aes(x=detection_threshold, y=f_hands, color=detection_threshold)) +
  theme_few(base_size=10) +
  geom_point() +
  geom_line() +
  ylim(0,1) +
...
ggpubr::ggarrange(f_hands, f_faces, nrow=1)
ggsave('figs_publication_final/figurea4b_fscore_by_thres.pdf', width = 7.2, height = 3)
ggsave('figs_publication_final/f_scores_by_thres.png', device = "png", width = 7, height = 3)

```



## A5/A6 OpenPose Error Analyses
```{r}
...
```{r det_perf_by_age, fig.width=6.5, fig.env="figure", fig.height=3.1, fig.cap="F-score of OpenPose hand and face detections in the gold sample by child and age. Size indicates number of detections age bin/child."}
...
p1 <- ggplot(prf_by_age_faces, aes(x=age_day_bin, y=f, col=child_id, size=num_detect)) +
  ...
  ggtitle("Face detections") + 
  scale_color_viridis(option='viridis', discrete=TRUE, begin=.2, end=.6) +
  theme(legend.position= 'none') 
...
ggsave('figs_publication_final/figurea5_fscore_by_age_child.pdf', width = 7.2, height = 4)
```
...
ggpubr::ggarrange(p1, p2, nrow=1)
ggsave('figs_publication_final/figurea6_fscore_by_location.pdf', width = 7.2, height = 3)
```


## A7 Face/hand detections relative to human annotations
...
ggplot(all_detections %>% filter(child_id %in% c('S','A')), 
       aes(x=age_day_bin, y=prop, 
           ...
           col=region)) +
  geom_point(alpha=.2) +
  # geom_point(alpha=.2, data = goldset) +
  geom_smooth(span=10, aes(weight = num_frames_total), show.legend = FALSE) +
  geom_smooth(data = goldset_adult_hands, span=10, aes(weight = num_frames_total), show.legend = FALSE, lty = 2, span=10, se = FALSE) +
  geom_smooth(data = goldset, span=10, aes(weight = num_frames_total), show.legend = FALSE, lty = 3, span=10, se = FALSE) +
...
ggsave('figs_publication_final/figurea7_humans_vs_op.pdf', width = 7.2, height = 4)

```


```{r}
summary_by_age_days_gold <- gold_sample %>%
  group_by(age_days,child_id) %>%
  summarize(num_faces = sum(face_present_ketan), 
            num_hands = sum(hand_present_ketan), 
            num_detect = n()) %>%
...
summary_by_age_days_gold$age_scale <- scale(summary_by_age_days_gold$age_days, scale = FALSE)[,1] / 30.3
```

```{r}

## add face vs. hands model on gold sample to sanity check
diff_model2_gold <- summary(lmer(prop ~ region*poly(age_scale, 2) + (1 | child_id), data = summary_by_age_days_gold))

xtable(diff_model2_gold$coefficients, digits=c(4,3,3,3,3,3),"Model coefficients from a linear mixed model predicting the proportion of faces and hands seen by infants based on human annotations.")


```


## A8 Activity by Location
...
```{r activities_by_lo, fig.width=6.5, fig.height=3.2, fig.cap="Counts of activities by location, including multilocation videos."}
...
meta_long %>% filter(Activity!="N/A", Activity!="Consequence", Activity!="Kicking",
  ...
  geom_tile() + theme_few(base_size=11) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# ggsave('figs_publication_final/figurea8_activities_by_location.pdf', width = 7.2 , height = 4)
...
ggsave('figs_publication_final/figurea8a_activities_by_single_location.pdf', width = 7.2, height = 4)
...
loc_freq_out = loc_freq
meta_long_1loc %>% 
  left_join(meta_long_1loc %>% group_by(Location) %>% summarise(loc_freq = n())) %>%
  group_by(Activity, Location) %>%
  summarise(n = n(),
            prop_loc = n / loc_freq) %>% 
  mutate(Location = factor(Location, levels=names(loc_freq_out))) %>%
...
ggsave('figs_publication_final/figurea8b_prob_activity_by_single_location.pdf', width = 7.2, height = 4)
```



## A9 Distribution of faces and hands in the visual field
...
As expected, faces tended to appear in the upper visual field in contrast to hands, which tended to be more centrally located (see Figure).
...
```{r faceHandDensity, fig.env="figure", fig.width=6.5, fig.height=9, fig.cap="Each panel shows the average position of faces and hands in the visual field in a videos from a given age range, i.e., videos when children in the dataset were were 5-31 months of age. Each dot represents the average position from one video within a given age range.", warning=F}
...
ggsave('figs_publication_final/figurea9_face_hand_density.pdf', width = 7, height = 7)
```


## A10 Face/hand detections by walking ability

Each parent in the SAYCam dataset completed the Ages and Stages Questionnaire (ASQ), which includes questions about their child's motor abilities, and we used the responses from this questionnaire to code the age (in months) at which each child became able to walk on their own.  We found considerable variability in the age at which each child achieved this milestone: S began walking after 10 months, A after 13 months, and Y after 16 months of age.

Below, we plot face and hand detections for each child as a function of when each child started walking independently.


```{r}
face_hand_by_age_milestones <- face_hand_by_age %>%
  filter(num_frames_total > 5000) %>%
  mutate(walking = ifelse((child_id=="S" & age_day_bin > 10) |
                       (child_id=="A" & age_day_bin > 13) |
                       (child_id=="Y" & age_day_bin > 16), 'Walking', 'Not Yet Walking')) %>%
  mutate(child_name = case_when(child_id =='A' ~ "Child A",
                      child_id == "S" ~ "Child S",
                      child_id == "Y" ~ "Child Y")) %>%
  select(prop_faces, prop_hands, num_frames_total, age_day_bin, child_name, walking) %>%
  pivot_longer(cols = c(prop_faces, prop_hands), names_to = "region", values_to = "prop")  %>%
    mutate(region = case_when(region =='prop_hands' ~ "Hands",
                      region == "prop_faces" ~ "Faces"))
         
```

```{r}
ggplot(face_hand_by_age_milestones,
       aes(x=age_day_bin, y=prop,
           size=log10(num_frames_total),
           col=walking)) +
  geom_point(alpha=.3) +
  ...
  xlab('Age (Months)') +
  ylim(0,.5) +
  facet_grid(region ~ child_name) +
  theme_few(base_size=10) +
  viridis::scale_color_viridis(name = "", option="C", discrete=TRUE, begin=0, end=.4) +
  theme(legend.text=element_text(size=10)) +
  guides(size = 'none') +
  theme(legend.position="bottom")

ggsave('figs_publication_final/figurea10_face_hand_by_motor.pdf', width=7.2, height=5)
```
