...
face_hand_by_age <- all_vid_data %>%
  ungroup() %>%
  tidyr::replace_na(list(faces_and_hands=0)) %>%c
...
In our viewing of the SAYCam videos and in other annotations available with the dataset, activities varied somewhat predictably by location: for example, eating tended to occur in the kitchen, whereas playtime was the dominant activity in the living room. However, every location was of course associated with multiple activities; see Appendix Figure \ref{fig:activities_by_loc} for a heatmap relating individual activities to locations in the dataset.
...
Second, we also found an unexpectedly high proportion of hands in infants' view, even when restricting the analysis to detections in the center field-of-view to make the viewpoints comparable to those of headcams used in prior work and to exclude the portion of the frame most likely to contain children's own hands.
