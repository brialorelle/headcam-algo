...
As we found that children's hands tended to occur in the lower visual field (see Figure Appendix \ref{fig:handLocation}), we thus re-analyzed the entire dataset while restricting our analysis to the center field of view, decreasing the proportion of hand detections from `r round(sum(all_vid_data$num_hands)/sum(all_vid_data$num_frames),2)*100`% to `r round(sum(all_vid_data$num_hands_center,na.rm=TRUE)/sum(all_vid_data$num_frames),2)*100`%, and face detections from `r round(sum(all_vid_data$num_faces)/sum(all_vid_data$num_frames),2)*100`% to `r round(sum(all_vid_data$num_faces_center,na.rm=TRUE)/sum(all_vid_data$num_frames),3)*100`%. 
...
## Variation with motor development
In a set of exploratory analyses, we examined how the onset of sitting, cruising, and walking in each individual child changed the proportion of faces and hands in view. Each parent completed the Ages and Stages Questionnaire (ASQ), and we coded the age at which each child reached each motor milestone. To do so, we used a mixed-effects regression with per-child random intercepts


We next examine whether the SAYcam participants' age of reaching particular developmental milestones (sitting, cruising, and walking) significantly influenced the amount of hands or faces that they saw, using a mixed-effects regression with per-child random intercepts.
[table with each child's age of onset per milestone?]
...
Second, we also found an unexpectedly high proportion of hands in infants' view, even when restricting the field-of-view to the center field-of-view to make the viewpoints comparable to those of headcams used in prior work. 
