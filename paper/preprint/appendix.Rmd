...
# use meta_long for multi-location videos
meta_long_1loc %>% filter(Activity!="N/A") %>%
  group_by(Activity, Location) %>%
  summarise(n = n()) %>% 
  ggplot(aes(x=Location, y=Activity, fill=n)) +
  geom_tile() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
#ggsave("activity_by_location.pdf", width=6, height=10)

# Talking Walk -> Taking Walk ?
# Taking Walk -> Walking?
```
