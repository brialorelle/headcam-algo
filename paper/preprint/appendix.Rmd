...
```{r}
# Evaluating prfs in center detections is challenging because we ONLY have ground truth on the full image -- so it's not that informative. 

# return_prf_short(gold_sample$hand_eval_ketan_center)
# return_prf_short(gold_sample$face_eval_ketan_center)
```

```{r, fig.width=6.5, fig.height=3.2, fig.cap="F-score of OpenPose hand and face detections in the gold sample by child and age. Size indicates number of detections age bin/child."}
...
```{r, fig.width=6.5, fig.height=3.0, fig.cap="F-score of OpenPose hand and face detections in the gold sample by location. Size indicates number of detections per location: minimum = 55 (car); maximum = 1753 (living room)."}
...
```{r, fig.width=6.5, fig.height=3.0, fig.cap="Counts of activities by location, including multilocation videos."}
...
We next examine whether the SAYcam participants' age of reaching particular developmental milestones (sitting, cruising, and walking) significantly influenced the proportion of hands or faces that they saw, using a mixed-effects regression with per-child random intercepts.
We tried including random slopes by age and milestone, but the models did not converge and/or were singular.
Thus, for each milestone the R syntax was: \texttt{prop_faces ~ age * milestone + (1 | child_id)}.

Children's age of achieving each milestone was dummy-coded (i.e., 0=milestone not reached; 1=milestone reached).
Participants S and A were already able to sit at 6 months of age, while Y began achieving this milestone at 8 months.
A was able to cruise at 9 months of age, while S reached that milestone at 10 months, with Y just behind.
Learning to walk showed the most variation, with S walking after 10 months, A after 13 months, and Y after 16 months of age.
...
```{r, include=F}
# need to also include main effect terms ("age" and "cruise")?
face_cruise_eff <- effects::effect(term="age:cruise", mod=face_cruise)
fc_dat <- data.frame(face_cruise_eff) %>% filter(cruise==0 | cruise==1)
fc_dat %>% ggplot(aes(x=age, y=fit, group=cruise, color=cruise)) +
  geom_line() +
  geom_ribbon(aes(x=age, ymin=lower, ymax=upper, fill=cruise), alpha= 0.3) +
  geom_point(data=face_hand_by_age_milestones, aes(x=age, y=prop_faces))
```
