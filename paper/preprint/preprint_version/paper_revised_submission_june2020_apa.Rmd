...
```{r}
## Read in data 
# load(here::here('data/preprocessed_data/all_vid_data_from_bbs_july192020_merged.RData')) # all detections
# load(here::here('data/preprocessed_data/all_centered_vid_data_from_bbs_july2020.RData')) # centered cropped

load(here::here('data/data_preprint/all_vid_data_from_bbs_june2020.RData')) # all detections
load(here::here('data/data_preprint/all_centered_vid_data_from_bbs_june2020.RData')) # centered cropped

```

```{r}
load(here::here('data/preprocessed_data/gold_sample_from_bbs_july192020.RData')) # gold sample detections
load(here::here('data/data_preprint/gold_sample_annotations2020-01-31.RData')) # human annotations
```

```{r} 
### How large is the dataset we're analyzing?
num_frames = sum(all_vid_data$num_detect)

fps=30
num_seconds = sum(all_vid_data$num_detect)/fps
...
load(here::here('data/data_preprint/child_adult_hand_annotations_by_frame.RData'))
# 
...
load(here::here('data/data_preprint/hand_annotations_2020-01-29.RData'))
...
summary_by_age <- all_vid_data %>%
  ...
  summarize(num_frames_total = sum(num_detect), num_faces = sum(num_faces), num_hands = sum(num_hands), prop_faces_summary = num_faces / num_frames_total, prop_hands_summary = num_hands / num_frames_total)
```

```{r}
## summary for centered detections
...
```{r}
## get out summary by age for goldset hand-labelled frames
...
summary_by_age_gold <- gold_sample %>%
  left_join(vid_info) %>%
  group_by(age_day_bin, child_id) %>%
  summarize(num_frames_total = n(), num_faces = sum(face_present_ketan), num_hands = sum(hand_present_ketan), prop_faces_summary = num_faces / num_frames_total, prop_hands_summary = num_hands / num_frames_total)
...
wide_gold <- summary_by_age_adult_hands_gold %>%
  gather(region, prop, prop_faces_summary, prop_hands_summary) %>%
...
#   gather(region, prop, prop_faces_center_summary, prop_hands_center_summary) %>%
#   mutate(approach = "centered",
#          region = ifelse(region == "prop_faces_center_summary","Face","Hands")) 
#   # filter(num_frames_total > 5000) # eliminate small data point that skews scaling 
```
...
ggplot(wide_uncropped, 
  ...
  geom_smooth(data = wide_gold, span=10, aes(weight = num_frames_total), show.legend = FALSE,
...
```{r}
## Appendix figure --- same trends in gold set
# ggplot(wide_gold, 
#        aes(x=age_day_bin, y=prop, 
#            size=log10(num_frames_total),
#            col=region)) +
...
As children's hands tended to occur in the lower visual field (see Figure \ref{fig:handLocation}), we thus re-analyzed the entire dataset while restricting our analysis to the center field of view, decreasing the proportion of hand detections from `r round(sum(all_vid_data$num_hands)/sum(all_vid_data$num_detect),2)*100`% to `r round(sum(all_vid_data_center$num_hands)/sum(all_vid_data_center$num_frames),2)*100`%, but only decreased face detections from `r round(sum(all_vid_data$num_faces)/sum(all_vid_data$num_detect),2)*100`% to `r round(sum(all_vid_data_center$num_faces)/sum(all_vid_data_center$num_frames),3)*100`%. This cropping likely removed both the majority of detections of children's own hands but also some detections of adult hands (see Figure \ref{fig:handLocation}), especially as OpenPose was biased to miss children's hands when they were in view. Nonetheless, within this modified field of view, we still observed more hand detections than face detections (see dashed lines in Figure \ref{fig:FacesAndHands}).


```{r eval=FALSE}
summary_by_age_days_centered <- all_vid_data_center %>%
  group_by(age_days,child_id) %>%
  summarize(num_faces = sum(num_faces), num_hands = sum(num_hands), num_detect = sum(num_frames)) %>%
  mutate(prop_faces = num_faces / num_detect, prop_hands = num_hands / num_detect) 
summary_by_age_days_centered$age_scale <- scale(summary_by_age_days_centered$age_days, scale = FALSE)[,1] / 30.3
...
faces_model_full_age_out = summary(lmer(prop_faces ~ age_scale + (1 | child_id), data = summary_by_age_days_centered))

# hands over age
hands_model_full_age_out = summary(lmer(prop_hands ~ age_scale + (1 | child_id), data = summary_by_age_days_centered))


# explicit modeling of faces vs. hands and interaction with age
# main effect of ages and regions; no interaction.
wide_centered_age_days <- summary_by_age_days_centered %>%
  gather(region, prop, prop_faces, prop_hands) %>%
  mutate(approach = "cropped",
         region = ifelse(region == "prop_faces","Face","Hands"))

diff_model_full_age_out = summary(lmer(prop ~ region*age_scale + (1 | child_id), data = wide_centered_age_days))
```
...
face_hand_by_location <- vid_data_by_location %>%
  ...
  summarize(num_detect = sum(num_frames),prop_faces = sum(num_faces)/num_detect, prop_hands = sum(num_hands)/num_detect,  prop_hands_center =  sum(num_hands_center)/num_detect, prop_faces_center = sum(num_faces_center)/num_detect)  %>%
  mutate(prop_faces_center = replace_na(prop_faces_center,0), prop_hands_center = replace_na(prop_hands_center,0)) %>%
  group_by(Location) %>%
  filter(num_detect >3000)
...
vid_data_by_location_A <- vid_data_by_location %>%
  filter(child_id == "A") %>%
  group_by(Location) %>%
  summarize(num_faces_by_loc = sum(num_faces), num_no_faces_by_loc = sum(num_detect)-num_faces_by_loc,
            num_hands_by_loc = sum(num_hands),  num_no_hands_by_loc = sum(num_detect)-num_hands_by_loc)  
...
Hands_A <- cbind(vid_data_by_location_A$num_faces_by_loc, vid_data_by_location_A$num_no_faces_by_loc)
