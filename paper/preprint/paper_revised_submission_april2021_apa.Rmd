...
<!-- FIXME - references not rendering for -->
...
Finally, we analyzed how these two sources of social information co-occurred, finding that faces/hands were jointly present in `r sum(all_vid_data$faces_and_hands,na.rm=TRUE)/sum(all_vid_data$num_frames)*100` percent of frames (see face hand-occurrences across age in Figure \ref{fig:faceHandDetailed}). 
To do so, we  calculated the number of frames in which infants saw faces and hands together relative to overall proportions of faces/hands that were detected for each child and age range. 
As shown in Figure \ref{fig:faceHandContingency}, we found that all three infants were more likely to see hands independently -- without the presence of a face -- than they were likely to see faces independently. That is, generally speaking when a face was present, a hand also tended to be present. 
...
<!-- However, in our previous analysis we found relatively equal proportions of complete, full-view faces across development (see e.g., Figure \ref{fig:faceHandDetailed}, where full view faces stayed relatively constant as a proportion of all detected faces).  -->
...
<!-- NEED TO LOOK AT PROPORTION OF FULL FACES IN ALL FACES -->
<!-- : in other words, these very close views of eyes experienced by younger infants were sometimes partial views. -->
