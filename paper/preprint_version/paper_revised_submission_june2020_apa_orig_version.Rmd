...
```{r}
load(here::here('data/data_preprint/gold_sample_from_bbs_june2020.RData')) # gold sample detections
...
vid_data_centered_by_location <- all_vid_data_center %>%
  mutate(age_day_bin = cut(age_days, bins, labels=round(bin_starts/30,1))) %>%
  mutate(age_day_bin = as.numeric(as.character(age_day_bin))) %>%
  left_join(meta) %>%
  filter(!is.na(Location)) %>%
...
vid_data_centered_by_location$Location = droplevels(vid_data_centered_by_location$Location) # get rid of unused levels (more than 1 location)

# merge locations that seem similar
levels_to_modify = levels(vid_data_by_location$Location)
levels_to_modify = levels(vid_data_centered_by_location$Location)
...
levels(vid_data_centered_by_location$Location) <- levels_to_modify
...
```{r}
face_hand_by_location_centered<- vid_data_centered_by_location %>%
  group_by(Location,child_id,age_day_bin) %>%
  summarize(prop_faces = sum(num_faces)/sum(num_frames), prop_hands = sum(num_hands)/sum(num_frames), num_detect = sum(num_frames))  %>%
  group_by(Location) %>%
