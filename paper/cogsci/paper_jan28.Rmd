...
# function to return prfs
 ...
 load('../../data_cogsci/gold_sample_annotations2020-01-31.RData') # gold_sample, pre-filtered
...
As has been observed in other studies on automated annotation of headcam data [e.g. @frank2013; @sanchez2018postural; @bambach2015lending], detection tasks that are easy in third-person video can be quite challenging in egocentric videos, due to difficult angles and sizes as well as substantial occlusion. For example, the infant perspective often contains non-canonical viewpoints of faces (e.g., looking up at a caregiver's chin) as well as partially-occluded or oblique viewpoints of both faces and hands. Furthermore, hand detection tends to be a harder problem than face detection [@Simon2017hand; @bambach2015lending]. We thus expected overall performance to be lower in these naturalistic videos than on either photos taken from the adult perspective or in egocentric videos in controlled, laboratory settings [e.g., @sanchez2018postural]. 
...
## summary stats for gold sample detections
summary_by_age_gold <- gold_sample %>%
  ...
  summarize(num_detect = length(face_present_ketan), prop_faces = sum(face_present_ketan) / num_detect,  prop_hands = sum(hand_present_ketan) / num_detect) 
...
##
wide_gold <- summary_by_age_gold %>%
  gather(region, prop, prop_faces, prop_hands) %>%
  mutate(approach = "gold", region = ifelse(region == "prop_faces","Face","Hands"))  %>%
  filter(!is.na(child_id)) # weird video
...
ggplot(wide_uncropped, 
  ...
  geom_smooth(data = wide_gold, aes(weight = num_detect),
...
As children's hands tended to occur in the lower visual field (see Figure \ref{fig:handLocation}), we thus re-analyzed the entire dataset while restricting our analysis to the upper 60% of the field of view, decreasing the proportion of hand detections from `r round(mean(d_all$hand_openpose),2)*100`% to `r round(mean(d_cropped$hand_detected),2)*100`%, but only decreased face detections from `r round(mean(d_all$face_openpose),2)*100`% to `r round(mean(d_cropped$face_detected),3)*100`%. This cropping likely removed both the majority of detections of children's own hands but does also remove some detections of adult hands (see Figure \ref{fig:handLocation}), especially as OpenPose was biased to miss children's hands when they were in view. Nonetheless, within this modified field of view, we still observed more hand detections than face detections (see dashed lines in Figure \ref{fig:FacesAndHands}).
...
d_by_location_cropped_mini_S <- d_by_location_cropped %>%
  group_by(Location) %>%
  mutate(num_detect = length(hand_detected))  %>%
  filter(num_detect > 0)  %>%
  filter(child_id == "S") %>%
  select(Location, face_openpose,hand_detected) # using cropped hands here

d_by_location_cropped_mini_A <- d_by_location_cropped %>%
  group_by(Location) %>%
  mutate(num_detect = length(hand_detected))  %>%
  filter(num_detect > 0)  %>% #
  filter(child_id == "A") %>%
  select(Location, face_openpose, hand_detected) 

## drop levels
d_by_location_cropped_mini_S$Location = droplevels(d_by_location_cropped_mini_S$Location)
d_by_location_cropped_mini_A$Location = droplevels(d_by_location_cropped_mini_A$Location)

## make smaller dataframes for just faces/hands
d_by_location_cropped_mini_S_face <- d_by_location_cropped_mini_S %>%
  select(Location, face_openpose)

d_by_location_cropped_mini_S_hand <- d_by_location_cropped_mini_S %>%
  select(Location, hand_detected)
  
d_by_location_cropped_mini_A_face <- d_by_location_cropped_mini_A %>%
  select(Location, face_openpose)
  
d_by_location_cropped_mini_A_hand <- d_by_location_cropped_mini_A %>%
  select(Location, hand_detected)

# output (runs chi-squared tests in summary)
faces_S_loc = summary(table(d_by_location_cropped_mini_S_face))
hands_S_loc = summary(table(d_by_location_cropped_mini_S_hand))
faces_A_loc = summary(table(d_by_location_cropped_mini_A_face))
hands_A_loc = summary(table(d_by_location_cropped_mini_A_hand))

# effect size estimates
# cramersV(table(d_by_location_cropped_mini_S_face))
# cramersV(table(d_by_location_cropped_mini_S_hand))
# cramersV(table(d_by_location_cropped_mini_A_face))
# cramersV(table(d_by_location_cropped_mini_A_hand))
