...
As children's hands tended to occur in the lower visual field (see Figure 2), we thus re-analyzed the entire dataset while restricting our analysis to the upper 60 percent of the field of view. When we excluded detections that occurred in the bottom 40% of the frame, this decreased the proportion of hand detections from `r round(mean(d_all$hand_openpose),2)`percent to `r round(mean(d_cropped$hand_detected),2)` percent, but only decreased face detections from `r round(mean(d_all$face_openpose),2)` to `r round(mean(d_cropped$face_detected),2)`. Nonetheless, within this modified field of view, we still observed, on average, more hand detections than face detections. 
...
```{r DetByLocation, fig.env="figure*", fig.pos = "h", fig.align = "center", fig.width=7, fig.height=3.5, fig.cap = "Proportion of faces and hands by location in which egocentric videos were filmed; data from individual children are plotted separately (location annotations were not available for Y). Data are only taken from the top 60 percent of the frames so as to minimize the contribution of children’s own hands. Each dot represents data from a week in which videos were filmed and are scaled by the number of frames in that datapoint."}
...
Indeed, a coarse analysis based the location the videos were filmed in already highlights the variability of the the social information in view, both across locations and between individual children. Even within a given, well-defined context---e.g., mealtime in kitchens---there was considerable variability. Indeed, there are at least three ways to feed a young child in a kitchen: 1) sitting in front of the child, facing them as they sit in a high chair; 2) sitting behind the child, holding them as they face outward, and 3) sitting side by side. Each of these positions offer the child differing degrees of visual access to face and hands, and likely tend to vary across individuals 

Thus, these analyses underscore how the questions of how, when, from whom, and what data we sample become critically important when we attempt to draw conclusions about the regularities of experience. Further, while unprecedented in size, this dataset still has many limitations. These videos only represent a small portion of the everyday experience of these three children, all of whom come from relatively privileged households in western societies and thus are not representative of the global population. Any idiosyncrasies in how and when these particular families chose to film these videos undoubtedly influences the variability seen here. Finally, without eye-tracking data, we still do not know if children are attending to the social information in their visual field. 
