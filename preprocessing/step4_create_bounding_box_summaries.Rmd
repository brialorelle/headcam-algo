...
```{r}
op_file_dir = here::here("data/op_outputs_per_vid_rds/")
...
for (vid_file in op_files) {
  vid_index = vid_index + 1
   vid_file_path = paste0(here::here(), '/data/op_outputs_per_vid_rds/', vid_file)
   ...
   #    filter(!is.na(height)) %>%
   #    filter(mean_confidence > 0) %>% # sanity
   ...
   this_vid_faces <- this_vid_data %>%
      filter(!is.na(height)) %>%
      filter(label=='face') %>%
      filter(mean_confidence > 0) 
   
   empty_vid=FALSE
   # empty_vid = (length(this_vid_data$num_detect)==0)
   # if (empty_vid) {
   #   print(vid_file)
   #  }
...
# 
# save(all_vid_bbs,file=here::here('data/preprocessed_data_2021/bounding_box_summaries_all.RData'))
...
all_faces <- all_faces %>%
 ...
 save(all_vid_bbs,file=here::here('data/preprocessed_data_2021/all_faces_summary.RData'))
