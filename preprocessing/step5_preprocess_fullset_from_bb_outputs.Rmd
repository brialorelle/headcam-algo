...
for (vid_file in op_files) {
   ...
   gold_sample_op<- gold_sample_op_all %>%
      left_join(gold_sample_op_center, by=c('vid_name','child_id','frame')) %>%
      mutate(hand_detected_center = replace_na(hand_detected_center, 'FALSE')) %>%
      mutate(face_detected_center = replace_na(face_detected_center, 'FALSE'))
   
   # compile vid-level summaries 
   if (vid_index == 1){
     all_gold_sample_frames_op <- gold_sample_op
     # gold_sample_all_dets <- gold_sample_raw_dets 
   }
   else if (vid_index > 1) {
   all_gold_sample_frames_op <- all_gold_sample_frames_op %>%
     full_join(gold_sample_op)
   
   # gold_sample_all_dets <- gold_sample_all_dets %>%
   #   full_join(gold_sample_raw_dets)
...
# Save it out
```{r}
# gold sample individual detections for each frame with position data
save(gold_sample_all_dets, file = here::here('data/preprocessed_data_2021/',paste0('gold_sample_all_indiivdual_dets.RData')))
