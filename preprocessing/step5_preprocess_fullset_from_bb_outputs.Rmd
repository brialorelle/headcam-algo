...
save(all_gold_sample_frames_op, file = here::here('data/preprocessed_data_2021/',paste0('gold_sample_from_bbs_2021_',dataset_type,'.RData')))
```
...
for (vid_file in op_files) {
   ...
   for (detection_thres in all_thresholds){
      ...
      gold_sample_op_this_thres <- this_vid_data %>%
        ...
        mutate(vid_name = str_split(vid_name,"[.]")[[1]][1]) %>%
        group_by(frame, vid_name, child_id) %>%
        summarize(hand_detected = sum(!is.na(mean_confidence[label=='hand']))>0,face_detected = sum(!is.na(mean_confidence[label=='face']))>detection_thres) %>%
...
save(all_gold_sample_frames_op, file = here::here('data/preprocessed_data_2021/',paste0('gold_sample_from_bbs_2021_',dataset_type,'all_thresholds.RData')))
