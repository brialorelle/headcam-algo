...
OUT_PATH = "/scratch/groups/mcfrank/Home_Headcam_new/bounding_boxes_with_face_info/"
...
for f in files[1:2]:
	...
	if not os.path.exists(OUT_PATH + vid_name + "_bounding_boxes_and_face_info.csv"):
		# try:
		...
		df.to_csv(os.path.join(OUT_PATH, vid_name+"_bounding_boxes_and_face_info.csv"), index=False)
		# and for high confidence
		df_hc = pd.DataFrame(df_hc)
		df_hc.columns = col_names
		df_hc.fillna('') # replace NaNs with emptiness
		df_hc.to_csv(os.path.join(OUT_PATH, vid_name+"_bounding_boxes_and_face_info_high_conf.csv"), index=False)
	# except:
