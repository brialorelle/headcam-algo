...
```{r}
f <- dir("turk_annotations_single/")
jf <- paste("turk_annotations_single/",f,sep="")
jd <- read_csv(jf)
...
jd$response = jd$`Answer.trial-1.label`

agreement <- jd %>%
  group_by(Input.img_src1) %>%
  summarize(agree = response[1] == response[2])
```
...
```{r}
write_csv(d.filtered,'turk_annotations_dec5.csv')
```
